...
// ========= bf ==========
async function getState(contract, node) {
  verbose('getState', {contract, node});
  const state = await api.bloc.state(contract.name, contract.address, node)
    .catch(function(e) {
 ...
 * @returns User
 */
async function createUser(name, password, isAsync, node) {
  verbose('createUser', {name, password, node});
  const address = await api.bloc.createUser({
      password: password,
    }, name, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
  ...
  const txResult = await fill(user, true, node);
  return user;
}
...
*/
async function fill(user, resolve, node) {
  verbose('fill', {user, resolve, node});
  const blocResult = await api.bloc.fill({ // FIXME - update to latest bloch
...
async function getUsers(node) {
  ...
  const result = await api.bloc.users(node);
  return result;
}
...
*/
async function getContractString(name, filename) {
  verbose('getContractString', {name, filename});
  const string = await importer.getBlob(filename);
  return string;
}
...
async function call(user, contract, methodName, args, value, doNotResolve, node) {
  ...
  var result = await api.bloc.call({
...
async function callMethod(user, contract, methodName, args, value, doNotResolve, node) {
  return await call(user, contract, methodName, args, value, doNotResolve, node);
}
...
*/
async function callList(user, txs, doNotResolve, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('callList', {user, txs, resolve, node});

  //callList: function(body, from, address) {
  var results = await api.bloc.callList({
      password: user.password,
      txs: txs,
      resolve: resolve,
    }, user.name, user.address, resolve, node)
...
*/
async function uploadContractString(user, contractName, contractSrc, args, doNotResolve, txParams, node) {
  const resolve = (doNotResolve) ? false : true;
  args = args || {};
  txParams = txParams || {};
  verbose('uploadContractString', {user, contractName, args, txParams, resolve, node});

  var result = await api.bloc.contract({
      password: user.password,
      src: contractSrc,
      args: args,
...
async function uploadContract(user, contractName, contractFilename, args, doNotResolve, txParams, node) {
  verbose('uploadContract', {user, contractName, contractFilename, args, doNotResolve, txParams, node});
  // get the source
  const contractSrc = await getContractString(contractName, contractFilename);
  // upload
  return await uploadContractString(user, contractName, contractSrc, args, doNotResolve, txParams, node);
}
 ...
 */
async function  uploadContractList(user, txs, doNotResolve, node){
  const resolve = doNotResolve ? false : true;
  verbose('uploadContractList', {user, txs, resolve, node})
  const results = await api.bloc.uploadList({
      password: user.password,
      contracts: txs,
      resolve: resolve
    }, user.name, user.address, resolve, node)
...
*/
async function query(query, node) {
  verbose('query', {query, node});
  const results = await api.search.query(query, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
...
*/
async function send(fromUser, toUser, value, doNotResolve, nonce, node) {
  verbose('send', {fromUser, toUser, value, doNotResolve, node});
  const resolve = (doNotResolve) ? false : true;
  const result = await api.bloc.send({
...
async function sendList(fromUser, txs, doNotResolve, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('sendList', {fromUser, txs, resolve, node});
  const results = await api.bloc.sendList({
      password: fromUser.password,
      txs: txs,
      resolve: resolve,
    }, fromUser.name, fromUser.address, resolve, node)
...
async function getBlocResult(hash, resolve, node) {
  verbose('getBlocResult', {hash, resolve, node});
  const result = await api.bloc.result(hash, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
async function transactionResult(hash, node) {
  verbose('transactionResult', {hash, node});
  const result = await api.strato.transactionResult(hash, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
async function compile(compileList, node) {
  verbose('compile', {compileList, node});
  const result = await api.bloc.compile(compileList, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
async function isSearchable(codeHash) {
  const results = await query(`contract?codeHash=eq.${codeHash}`);
  return (results.length > 0);
}

async function isCompiled(contractName) {
...
async function getAccount(address, node) {
   ...
   const accounts = await api.strato.account(address, node)
     .catch(function(e) {
       throw (e instanceof Error) ? e : new HttpError(e);
     });
 ...
 async function getBalance(address, accountIndex, node) {
...
async function waitQuery(queryString, count, timeoutMilli, node) {
  ...
  for (var i = 0; i < retries ; i++) {
    // query
    var results = await query(queryString, node);
    ...
    await promiseTimeout(sleep);
  }
  // retries exceeded - timeout
  throw new Error(`waitQuery: timeout ${timeoutMilli}ms exceeded`);
}

async function waitTransactionResult(hash, timeoutMilli, node) {
  ...
  for (var i = 0; i < retries ; i++) {
    // transactionResult
    var results = await transactionResult(hash, node);
    // got one - done
    if (results.length > 0) {
      return results;
    }
...
async function compileSearch(searchableArray, contractName, contractFilename, node) {
  // get the contract string
  const source = await getContractString(contractName, contractFilename);
  const compileList = [{
  ...
  // compile
  const compileResults = await compile(compileList, node);
...
async function getAbi(contract, node) {
  verbose('getAbi', {contract, node});
  const abi = await api.bloc.abi(contract.name, contract.address, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return abi;
...
async function getAbiEnums(contract, node) {
  verbose('getAbiEnums', {contract, node});
  const abi = await getAbi(contract, node);
  var enums = {};
