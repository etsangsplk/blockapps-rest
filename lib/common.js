...
exports.assert.shouldThrowRest = function* (func, status, message = '') {
  let result;
  try {
    result = yield func();
  } catch (err) {
    chai.assert.equal(err.status, status, 'Should have thrown REST status ' + status);
    return; // got the right error status - all good
  }
  // should not have succeeded
  chai.assert(false, `${message}. Should have thrown rest error: ${status} instead got ${JSON.stringify(result)}`);
}
